...
//
    ...
    ///hmm...not  sure i want this one to be liveData
    ...
    LiveData<Call<AaqMovieTrailerList>> doGetMovieTrailers(@Path("id") int  id);

    //GetMovieTrailers by Lang
    @GET("/3/movie/{id}/videos?api_key=" + R.string.api_key+ "&language={lang}")
    LiveData<Call<AaqMovieTrailerList>> doGetMovieTrailers(@Path("id") int id, @Query("language") String lang);
    ...
    LiveData<Call<AaqReviewsList>> doGetReviewsList(@Path("id") int  id);

    //getReviews by lang
    @GET("3/movie/{id}/reviews?api_key=" + R.string.api_key + "&language={lang}")
    LiveData<Call<AaqReviewsList>> doGetReviewsList(@Path("id") int id, @Query("lang") String lang);
